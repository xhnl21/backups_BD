...
class Backup_Database {
     ...
     */
    public function __construct($host, $username, $passwd, $dbName, $charset = 'utf8', $WEB_NAME) {
        $this->host                    = $host;
        ...
        if( $this->conn ){
            ...
            $this->web_name                = (isset($WEB_NAME)) ? $WEB_NAME : '-';;
            $this->backupFile              = 'backup-'.$this->web_name.'-'.$this->dbName.'-'.date("Ymd_His", time()).'.sql';
    ...
    protected function initializeDatabase() {
        try {
            ...
            if (mysqli_connect_errno()) {
                // throw new Exception('ERROR connecting database: ' . mysqli_connect_error()); // muestar el msj de error
                // die(); // rompe el ciclo
                return false;
            }
    ...
    public function backupTables($tables = '*') {
        try {
            ...
            if ($this->gzipBackupFile) {
                ...
                if(!empty($this->backupDir) && !empty($this->backupFile)){
            ...
            print_r($e->getMessage());
            return false;
            //$this->obfPrint('fallo al guardar');
        }
        return true;
        //$this->obfPrint('COMPLETADO');
    }
    /**
     * Save SQL to file
     * @param string $sql
